...
const errorMessage = ref('reset.weak_password');
const isLoading = ref(false);

const rules = {
    required: value => !!value || i18n.t('login.errors.required'),
    confirm: value => value === password.value || i18n.t('forgot.errors.mismatch')
};
...
const validateAndSubmit = async () =>
{
    ...
    // Submit form
    let result = await Users.resetPassword(route.params.code as string, password.value);

    if (!result.code_check) {
        errorMessage.value = 'reset.code_check';
    }

    if (result.code_check && result.password_check) {
