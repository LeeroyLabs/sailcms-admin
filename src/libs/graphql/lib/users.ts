...
import { LoginResult, PasswordChangeResult, User, UserData, UserListArguments, UserListing } from "../types/users";
...
export class Users
{
     ...
     *
     */
    public static async createUser(data: UserData, forceNewPassword: boolean): Promise<string>
    {
        const client = new Client();
     ...
     *
     */
    public static async updateUser(id: string, data: UserData): Promise<string>
    {
        const client = new Client();
    ...
    public static async deleteUsers(ids: string[]): Promise<boolean>
    {
        ...
        let result = await client.query(gql`${query}`, {ids: ids});
    ...
    private static async changeStatus(ids: string[], status: boolean): Promise<boolean>
    {
        const client = new Client();
        let query = UserQueries.changeUserStatus;

        let result = await client.query(gql`${query}`, {ids: ids, status: status});
